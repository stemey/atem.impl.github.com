...
public class SingleAttributeViewCreator implements ViewCreator {
	...
	public Serializable[] getKeys(Object entity) {
		...
		for (Attribute attribute : entityType.getAttributes()) {
			if (attribute instanceof SingleAssociationAttribute) {
				...
				IdentityService service = singleAttribute.getTargetType()
						.getService(IdentityService.class);
				if (service != null) {
					Serializable id = service.getId(
							singleAttribute.getTargetType(), value);
					Serializable key = getKeyForParameter(new Object[] {
							attribute, value });
					if (key != null) {
						keys.add(key);
					}
	...
	public Serializable getKeyForParameter(Object[] parameter) {
		...
		IdentityService service = singleAttribute.getTargetType()
			...
			if (service != null) {
				Serializable id = service.getId(
						singleAttribute.getTargetType(), targetEntity);
				return new ArrayKey(new Serializable[] {
						id,
						attribute.getTargetType().getCode() + "."
								+ attribute.getCode() });
			}
